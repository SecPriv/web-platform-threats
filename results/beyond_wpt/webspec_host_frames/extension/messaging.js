...
var code = function setup_logs(w){
    ...
    function log_to_extension(type, args=[], ret=null, ts=Date.now()){
        ...
        let content = { 'isSecure': w.isSecureContext,
                        ...
                        // 'frame': w.frameElement, This can be an Object, which will make `content` not serializable and, thus, empty
                        'type': type,
                        'args': args,
                        'ret': ret,
                        'csFramed': csFramed()
    ...
    var domain_setter_orig = w.document.__lookupSetter__("domain").bind(w.document);
    ...
    w.cookie_getter_orig = cookie_getter_orig
    w.cookie_setter_orig = cookie_setter_orig
