...
browser.cookies.getAll({domain: 'web-platform.test'}).then((cdom1) => {
    browser.cookies.getAll({domain: 'not-web-platform.test'}).then((cdom2) => {
        ...
        prev_cookies = JSON.stringify(cookies)
    })
})
...
function poll_cookies(){
    ...
    browser.cookies.getAll({domain: 'web-platform.test'}).then((cdom1) => {
        browser.cookies.getAll({domain: 'not-web-platform.test'}).then((cdom2) => {
            let cookies = cdom1.concat(cdom2)
            if (JSON.stringify(cookies) != prev_cookies){
                cookie_logs.push({ts: dt, cookies:JSON.stringify(cookies), prev: prev_cookies})
                prev_cookies = JSON.stringify(cookies)
            }
        })
    })
...
function convert_csevents(cookies) {
    function dff(obj1, obj2) {
        obj1 = typeof obj1 == 'string'? JSON.parse(obj1) : obj1
        obj2 = typeof obj2 == 'string'? JSON.parse(obj2) : obj2
        const result = {};
    ...
    cookies.forEach(function(el, idx) {
        ...
        try{
            ...
            Object.keys(d).forEach(function(e, i){
                // console.log(e, d[e])
                if (!d[e]){
                    res[el.ts].push({removed: true, cookie: JSON.parse(el.prev)[e]})
                } else if(Object.keys(d[e]).length > 0){ 
                    // event, removed=false
                    res[el.ts].push({removed: false, cookie: JSON.parse(el.cookies)[e]})
                }
            })
        } catch(error){
            console.log(error)
...
async function(request, sender, sendResponse) {
    ...
    switch(request.type) {
        case "START":
            start = request.ts;
            // we may have repeated starts now, count the first only
        ...
        case "DOWNLOAD":
            ...
            console.log("DOWNLOADING EVENTS", events.network.requests)
            let blob = new Blob([JSON.stringify(out)], {type: 'application/json'});
            let u = URL.createObjectURL(blob);

            browser.downloads.download({url: u, filename: `${Date.now()}_firefox.json`});
